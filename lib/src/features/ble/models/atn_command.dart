...
class AtnCommand with _$AtnCommand {
  ...
  factory AtnCommand.control(ControlCommand command, { ack = false }) => AtnCommand.command(
      code: ack ? 0xE1 : 0xE0, transactionId: TIDGenerator.next(),
      values: [command.code]
  );
  ...
  factory AtnCommand.setConfig({
    ...
    featureFlags = FeatureFlags.timeoutReport, ack = false
  }) => AtnCommand.command(
      code: ack ? 0xF1: 0xF0, transactionId: TIDGenerator.next(),
  ...
  factory AtnCommand.train({
    ...
    required Sensor sensor, required Timeout timeout, ack = false
  }) => AtnCommand.command(
      code: ack ? 0xC1 : 0xC0, transactionId: TIDGenerator.next(),
  ...
  List<int> get data => [code, transactionId, ...values];
}